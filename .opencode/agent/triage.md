...
**Only** add if it's specifically about the Desktop application or `aictrl web` view. **Do not** add for terminal, TUI, or general aictrl issues.
...
**Only** add if the issue mentions "zen" or "aictrl zen" or "aictrl black".

If the issue doesn't have "zen" or "aictrl black" in it then don't add zen label

#### core

Use for core server issues in `packages/aictrl/`, excluding `packages/aictrl/src/cli/cmd/tui/`.
...
TUI (`packages/aictrl/src/cli/cmd/tui/...`):
...
Core (`packages/aictrl/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for aictrl server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
