...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with aictrl...`)
  const prompt = [
  ...
  try {
    await $`aictrl run -m aictrl/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  aictrl failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with aictrl")
  return true
}
