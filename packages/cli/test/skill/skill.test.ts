...
test("discovers skills from .aictrl/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".aictrl", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".aictrl", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".aictrl", "skill", "dir-skill")
...
test("discovers multiple skills from .aictrl/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".aictrl", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".aictrl", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".aictrl", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.AICTRL_TEST_HOME
  process.env.AICTRL_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.AICTRL_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const aictrlSkillDir = path.join(dir, ".aictrl", "skill", "agent-skill")
      const aictrlSkillsDir = path.join(dir, ".aictrl", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(aictrlSkillDir, "SKILL.md"),
        `---
name: aictrl-skill
description: A skill in the .aictrl/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(aictrlSkillsDir, "SKILL.md"),
        `---
name: aictrl-skill
description: A skill in the .aictrl/skills directory.
---
