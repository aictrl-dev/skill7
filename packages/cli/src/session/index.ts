import { Slug } from "@skill7-ai/util/slug"
...
export namespace Session {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".skill7", "plans")
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like Skill7's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
