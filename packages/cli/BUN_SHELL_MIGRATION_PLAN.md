...
Replace runtime Bun shell template-tag usage in `packages/aictrl/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/aictrl/src` is removed except:
